...
class PlannerType(Enum):
    ...
    RANDOM_GOAL = 5
    # Add more!
    STOP_AND_WAIT = 6  # New planner type for stopping when artifact detected
    APPROACH_ARTIFACT = 7  # Planner type for the robot to approach the detected artifact
...
class CaveExplorer:
    def __init__(self):
        ...
        self.stop_on_detection_ = False  # Planning 2, part 2
    ...
    def image_callback(self, image_msg):
        # Convert the image message to a writable OpenCV image
        ...
        if num_detections > 0:
            ...
            if self.move_base_action_client_.get_state() == actionlib.GoalStatus.ACTIVE:
                self.move_base_action_client_.cancel_goal()
                rospy.loginfo('Canceling current goal - artifact detected')
    ...
    def planner_stop_and_wait(self, action_state):
        ...
        if action_state == actionlib.GoalStatus.ACTIVE:
            self.move_base_action_client_.cancel_goal()
            rospy.loginfo('Stopping robot - artifact detected!')
    ...
    def planner_approach_artifact(self, action_state):
        if action_state != actionlib.GoalStatus.ACTIVE:
            ...
            # Move 1 meter closer to detected artifact
            pose_2d = Pose2D()
            pose_2d.x = current_pose.x + math.cos(current_pose.theta)
            pose_2d.y = current_pose.y + math.sin(current_pose.theta)
            pose_2d.theta = current_pose.theta  # maintain current orientation
            ...
            rospy.loginfo('Approaching artifact...')
            self.move_base_action_client_.send_goal(action_goal.goal)
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            # if self.artifact_found_:  # Check for artifact first
            ...
            if self.artifact_found_:
                if self.planner_type_ != PlannerType.APPROACH_ARTIFACT:  # If we're not already approaching
                    if action_state == actionlib.GoalStatus.SUCCEEDED or self.planner_type_ != PlannerType.STOP_AND_WAIT:
                        self.planner_type_ = PlannerType.STOP_AND_WAIT
                    else:
                        self.planner_type_ = PlannerType.APPROACH_ARTIFACT  # After stopping, start approaching
            elif not self.reached_first_artifact_:
                self.planner_type_ = PlannerType.GO_TO_FIRST_ARTIFACT
