...
class CaveExplorer:
    def __init__(self):
        ...
        self.model_path_ = rospy.get_param("~model_path", "/home/vincent/git/cave_explorer/config/model_3.pt")
    ...
    def image_callback(self, image_msg):
        ...
        if len(results) > 0:
            ...
            rospy.loginfo(f"Objects detected: {results[0].names}")

            # Annotate the image with bounding boxes
            annotated_image = results[0].plot()  # Plots bounding boxes on the image
        else:
            self.artifact_found_ = False
